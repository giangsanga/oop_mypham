...
int mypham::kiemTraArr(string f_txt){//Kiểm tra mảng sanpham đã chứa bao nhiêu sản phẩm, f_txt <- data.txt;
    int n = 0;
    string line;
    ifstream file(f_txt);
...
void mypham::ghiFile(mypham sanpham[], int n){
...
void mypham::docFile(mypham sanpham[], const string& f_txt){
...
void mypham::Them(mypham sanpham[]){
    int n = kiemTraArr("data.txt");
    fflush(stdin);
    cout << "Nhap ma hang: ";
...
int mypham::timKiem(mypham sanpham[], string mahang, int n){//Tìm kiếm sản phẩm nằm ở phần tử số bao nhiêu trong mảng sanpham[];
    for(int i = 0; i < n; i++){
        if(sanpham[i].maHang == mahang) return i;
    }
}
void mypham::Xoa(mypham sanpham[]){//Ham xoa thong tin
    int n = kiemTraArr("data.txt");
...
void mypham::Chinh(mypham sanpham[]){//
    cout << "----------------------Chinh sua----------------------------\n";
...
void mypham::thanhToan(mypham sanpham[]){
    cout << "-------------------Thanh toan----------------------\n";
    int choice = 1;
    do{
        cout << "Nhap ma hang: ";
...
void mypham::xemThongTinTatCaSanpham(mypham sanpham[]){
...
int menuchinh(mypham sanpham[]){
    ...
    switch(choice){
        ...
        return 1;
        case 3:
        mypham::xemThongTinTatCaSanpham(sanpham);
        return 1;
        break;
        case 0:
        return 0;
